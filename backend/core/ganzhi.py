...
def _reference_lunar(year: int, month: int, day: int, hour: int = 0):
    """返回指定公历时刻对应的 lunar-python Lunar 对象。"""
    return Solar.fromYmdHms(year, month, day, hour, 0, 0).getLunar()


def _year_ganzhi(year: int, month: int, day: int, hour: int = 0) -> str:
    """按立春精确交接时刻计算年柱。"""
    return _reference_lunar(year, month, day, hour).getYearInGanZhiExact()


def _month_ganzhi(year: int, month: int, day: int, hour: int = 0) -> str:
    """按每月“节”的精确交接时刻计算月柱。"""
    return _reference_lunar(year, month, day, hour).getMonthInGanZhiExact()


def _day_ganzhi(year: int, month: int, day: int, hour: int = 0) -> str:
    """计算日柱；晚子时采用民用日期不换日（sect 2）约定。"""
    return _reference_lunar(year, month, day, hour).getDayInGanZhiExact2()
...
def get_ganzhi_by_date(
    ...
    - 时柱：按上述日柱的日干使用五鼠遁。
    """
    # 先借助 datetime 产生清晰、稳定的输入错误。
    datetime.datetime(year, month, day, hour)

    year_gz = _year_ganzhi(year, month, day, hour)
    month_gz = _month_ganzhi(year, month, day, hour)
    day_gz = _day_ganzhi(year, month, day, hour)
